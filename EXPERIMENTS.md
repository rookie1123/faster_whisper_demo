...
同一段朗读稿（`samples/reading_script.txt`，116 字），由四名成员各自用手机录制，
评测条件完全相同：CPU + int8 量化、`beam_size=5`、开启 VAD、不加任何提示词。

| 说话人 | 音频 | tiny 规范化 CER | small 规范化 CER |
|---|---|---|---|
| speaker_fujian | `samples/speaker/speaker_fujian.mp3` | 36.2% | 6.9% |
| speaker_daiyipeng | `samples/speaker/speaker_daiyipeng.m4a` | 44.0% | 6.0% |
| speaker_huangsiyang | `samples/speaker/speaker_huangsiyang.m4a` | 56.9% | 8.6% |
| speaker_gaojinglin | `samples/speaker/speaker_gaojinglin.m4a` | 17.2% | 7.8% |
| **四人极差** | | **39.7 个百分点** | **2.6 个百分点** |

**结论一：small 的表现跨说话人很稳定。**
四个人从 6.0% 到 8.6%，极差只有 2.6 个百分点。说明它的剩余错误主要来自
内容本身的难度（人名、英文术语、数字），而不是"换个人念就变差"。

**结论二：tiny 的表现极不稳定，极差接近 40 个百分点。**
同样一段文字、同一个模型，最好的一次 17.2%、最差的一次 56.9%，**相差三倍以上**。

**结论三（实践含义）：说话人多样时不能用 tiny。**
tiny 慢不慢不是问题（它最快），问题是**你没法预期它这次是好还是坏**。
small 虽然慢 4 到 5 倍，但结果稳定、可预期——对要交付的工具来说，
"稳定"比"平均快"更重要。

> 注：本轮四段录音是同一段文字，所以这里验证的是**跨说话人**的稳定性，
> 不能推广到"跨内容"。不同内容之间的表现差异需要另做实验（见待办）。
